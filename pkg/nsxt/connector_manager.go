...
import (
	...
	nsxtcfg "github.com/martinweindel/cloud-controller-manager-vsphere-with-k8s/pkg/nsxt/config"
...
func NewConnectorManager(nsxtConfig *nsxtcfg.NsxtConfig) (*ConnectorManager, error) {
	cm := &ConnectorManager{}
	if nsxtConfig == nil {
		return cm, nil
	}
	cm.config = &nsxtConfig.NSXT
	url := fmt.Sprintf("https://%s", nsxtConfig.NSXT.Host)
	var securityCtx *core.SecurityContextImpl
	securityContextNeeded := true
	nsxt := &nsxtConfig.NSXT
	if len(nsxt.ClientAuthCertFile) > 0 {
		securityContextNeeded = false
	}

	if securityContextNeeded {
		securityCtx = core.NewSecurityContextImpl()
		if len(nsxt.VMCAccessToken) > 0 {
			if nsxt.VMCAuthHost == "" {
				return nil, fmt.Errorf("vmc auth host must be provided if auth token is provided")
			}

			apiToken, err := getAPIToken(nsxt.VMCAuthHost, nsxt.VMCAccessToken)
			if err != nil {
				return nil, err
			}
		...
		} else if nsxt.User != "" && nsxt.Password != "" {
			securityCtx.SetProperty(security.AUTHENTICATION_SCHEME_ID, security.USER_PASSWORD_SCHEME_ID)
			securityCtx.SetProperty(security.USER_KEY, nsxt.User)
			securityCtx.SetProperty(security.PASSWORD_KEY, nsxt.Password)
		}
	}

	tlsConfig, err := getConnectorTLSConfig(nsxt.InsecureFlag, nsxt.ClientAuthCertFile, nsxt.ClientAuthKeyFile, nsxt.CAFile)
	if err != nil {
		return nil, err
	}
